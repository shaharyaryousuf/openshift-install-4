...
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -ldflags '-w -extldflags "-sta                                                                                        tic"' -o terraform-provider-esxi_`cat version`
sleep 300
echo 'go installed and configured'
dnf -y install libnsl
[root@bastion openshift-install-4]# cat installbation.sh
#!/bin/bash
# installBastion.sh
#
# Execute this script as root
